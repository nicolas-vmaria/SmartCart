...
export default function Login() {
    ...
    return (
        <main className="flex justify-center items-center min-h-screen bg-gray-50">
            ...
            <Link to={"/"}><img src={logo} alt="Peccato Logo" className='w-40 h-auto absolute left-4 sm:left-10 top-4' /></Link>
            <div className="flex flex-col items-center justify-center bg-white w-full max-w-lg mx-4 rounded-2xl p-8 sm:p-12 shadow-xl">
                ...
                <form onSubmit={handleSubmit} className='flex flex-col gap-5 w-full '>
                    <div>
                        <label htmlFor="" className='text-sm font-bold ml-1'>E-mail:</label>
                        <input type="email" placeholder='Digite seu e-mail' onChange={(e) => setEmail(e.target.value)} required className='bg-white w-full h-12 p-5 box-border rounded-xl border-1  border-gray-200' />
                    </div>
                    <div>
                        <label htmlFor="" className='text-sm font-bold ml-1 mr-1'>Senha:</label>
                        <input type="password" onChange={(e) => setSenha(e.target.value)} placeholder='Digite sua senha' required className='bg-white w-full h-12 p-5 box-border rounded-xl border-1  border-gray-200' />
                        <Link to={"/forgot-password"} className='text-sm hover:underline'>Esqueceu a senha?</Link>
                    </div>

                    <button disabled={loading} className='bg-verde-escuro text-white h-12 rounded-xl transition-all duration-100 hover:-translate-y-2 hover:shadow-xl active:translate-y-0 active:bg-verde-claro active:text-verde-escuro cursor-pointer disabled:opacity-60 disabled:cursor-not-allowed disabled:translate-y-0 disabled:shadow-none flex items-center justify-center gap-2'>
                        {loading && <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" />}
                        {loading ? 'Entrando...' : 'Login'}
                    </button>

                    <p className='text-sm'>Não tem uma conta? <Link to={'/register'} className='font-bold hover:underline hover:text-red cursor-pointer'>Cadastre-se</Link></p>
                </form>
