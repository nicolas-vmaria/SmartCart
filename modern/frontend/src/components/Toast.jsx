...
export default function Toast({ message, type = 'success', onClose }) {
    ...
    return (
        <div role="status" aria-live="polite" className={`fixed bottom-6 right-6 flex items-start gap-3 ${container} rounded-xl px-5 py-4 shadow-2xl w-80`}>
            <Icon size={20} className={`${iconClass} shrink-0 mt-0.5`} />
            <p className={`text-sm font-medium flex-1 ${titleClass}`}>{message}</p>
            <button onClick={onClose} aria-label="Fechar notificação" className="text-gray-300 hover:text-gray-500 transition-colors cursor-pointer shrink-0 mt-0.5">
                <X size={16} />
            </button>
        </div>
    )
